...
export class Controller {
  ...
  getResource(req: Request, res: Response): void {
    ...
    if (!resource) {
      ...
      res.status(400).send('Malformed resource query parameter, it must be in the format `acct:user@domain`');
    } else if (resource.replace(/.*@/, "") !== process.env.DOMAIN) {
      res.status(400).send(`Supplied resource resides at a different domain, you can only view resources residing at http(s)://${process.env.DOMAIN} at this endpoint.`);
    } else {
